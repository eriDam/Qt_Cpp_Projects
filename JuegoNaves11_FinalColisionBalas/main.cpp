/*Parte 11 https://www.youtube.com/watch?v=p9uG2Ze7D7c*/
#include <stdio.h>
#include <windows.h>
...
public:
    ...
    //funcion que nos devuelve las vidas de la nave player
...
int main()
{
   ...
   // AST ast1(10,4),ast2(4,8), ast3(15,10);
    ...
    for(int i = 0; i < 5; i++){
        AS.push_back(new AST(rand()%74 + 3, rand()%5 + 4));
    }
    ...
    */
    ...
    while (!game_over) { //Se ejecutara siempre que el argumento sea true q no sea game over
        //Vamos a imprimir los puntos en pantalla
        ...
        }//Fin for recorrer lista de asteroides

        //Detectar la colisión de las balas con los asteroides
        //Recorrer toda la lista de los asteroides, check sus coordenadas
        //y compararlas con las coordenadas de las balas
        //Tendremos que recorrer tambien las listas de las balas
        for(itAs = AS.begin(); itAs != AS.end(); itAs++){
            for(it = B.begin(); it !=B.end(); it++){
                //condicion para detectar que las coordenadas se solapen
                //
                if((*itAs)->X() == (*it)->X() && ( (*itAs)->Y() +1 == (*it)->Y() || (*itAs)->Y() == (*it)->Y() ))
                {
                    //en caso de que exista esta colision borramos la bala
                    //gotoxy nos situamos en la posicion de la bala y usamos el *it que es el q controla lña bala
                    //
                    gotoxy((*it)->X(), (*it)->Y()); printf(" ");
                    //Hay que borrar al iterador,  este elem de la lista no debe existir por que ha chocado y ha desaparecido
                    delete(*it);
                    //Se pierde el hilo, al it lo pasaremos al siguiente elem de la lista
                    it = B.erase(it);

                    //Hay que borrar el asteroide con el que ha chocado, qeu ya no esté hay
                    //Y a la vez hay que crfear otro nuevo asteroide qeu lo reemplace, que aprezca hasta arriba de nuestro limite
                    //con alguna coord en X al azar para que no se pierda el numero de asteroides, si no lo volvemos a crear, a la hora de
                    //destruir uno este ya no se repone, podriamos quedarnos sin asteroides  si los destruimos todos
                    AS.push_back(new AST(rand()%74 +3, 4));

                    //hay que borrar el asteroide, puede que se quede pintado cuando no exista
                     gotoxy((*itAs)->X(), (*itAs)->Y()); printf(" ");

                     //Borrar el asteroide que ha chocado con la bala
                     delete(*itAs);
                     //Se pierde el hilo, al itAs lo pasaremos al siguinete elem de la lista
                     itAs = AS.erase(itAs);

                     //cuando se produzca la colisión entre bala y asteroide, puntos aumenta en 1
                     puntos+=1;
        ...
        //Si gameOver es true se acabará el juego
    ...
    //Imprimo en pantalla mensaje de Game Over y creditos
    printf("\n\n%c%c%cGAME OVER!!!",173,173,173);
    printf("\nCreated by eriDev\nPlaying with c++\n:) \n");
